...
def show_proxy_input():
    ...
    save_button = tk.Button(popup, text="Save", command=lambda: save_proxy_input(popup, proxy_entry.get()))
...
def show_cookie_input():
    ...
    # 创建一个按钮，用于保存用户输入并关闭弹出窗口
    save_button = tk.Button(popup, text="Save", command=lambda: save_cookie_input(popup, cookie_entry.get()))
